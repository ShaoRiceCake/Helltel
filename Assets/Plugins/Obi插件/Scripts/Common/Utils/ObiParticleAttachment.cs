...
namespace Obi
{
    ...
    public class ObiParticleAttachment : MonoBehaviour
    {
        ...
        public void BindToTarget(Transform transform)
        {
            m_Target = transform;
            ...
            if (m_Target != null && m_ParticleGroup != null && m_Actor.isLoaded)
            {
                ...
                for (int i = 0; i < m_ParticleGroup.Count; ++i)
                {
                    ...
                    if (particleIndex >= 0 && particleIndex < m_Actor.solverIndices.count)
                    {
                        ...
                        m_SolverIndices[i] = m_Actor.solverIndices[particleIndex];
                        m_PositionOffsets[i] = Vector3.zero;

                        Vector3 targetPosition = m_Target.TransformPoint(m_PositionOffsets[i]);
                        m_Actor.solver.positions[m_SolverIndices[i]] = m_Actor.solver.transform.InverseTransformPoint(targetPosition);
                    }

                    if (m_Actor.usesOrientedParticles)
                    {
                        for (int k = 0; k < m_ParticleGroup.Count; ++k)
                        {
                            m_OrientationOffsets[k] = Quaternion.identity;
                            Quaternion targetRotation = m_Target.rotation * m_OrientationOffsets[k];
                            m_Actor.solver.orientations[m_SolverIndices[k]] = targetRotation;
                        }
                    }
                }
            }
