...
public abstract class PlayerControl_FootControl : PlayerControl_BaseControl
{
    ...
    protected bool TryLiftFoot()
    {
        ...
        _hasTriggeredLockEvent = false;
        shouldShowParticles = true; // 抬起腿时显示粒子
        return true;
    }
    ...
    protected void ReleaseFoot()
    {
        ...
        _hasTriggeredLockEvent = false;
        shouldShowParticles = false; // 放下腿时隐藏粒子
    ...
    protected void LockFoot()
    {
        ...
        shouldShowParticles = false; // 锁定腿时隐藏粒子
    ...
    {
        if (controlParticleEffect == null) return;
    
        if (shouldShowParticles)
        {
            if (!controlParticleEffect.isPlaying)
            {
                controlParticleEffect.transform.position = footObject.transform.position;
